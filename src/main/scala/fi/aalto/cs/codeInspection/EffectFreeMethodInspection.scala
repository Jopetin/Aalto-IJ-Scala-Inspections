...
import org.jetbrains.plugins.scala.lang.psi.api.expr.{ScArgumentExprList, ScExpression, ScMethodCall, ScParenthesisedExpr, ScReferenceExpression}
import org.jetbrains.plugins.scala.lang.psi.api.statements.ScFunctionDefinition

/**
 * Inspection used to check whether results from methods with given annotation are used in any way
 *
 * Essentially when finding a method call to a method with the given annotation, it checks whether it is the last child of its parent in the Psi tree.
 * Annotation is identified based on its name, which can be configured in the message bundle.
 * If it is set to a variable or returned by the function, the last line will be the method call itself.
 * Cases where the method return value is used as a parameter or chained to an another method call are checked through the class of the parent element.
 */
        ...
        case ref: ScReferenceExpression =>
          ref.resolve() match
            case func: ScFunctionDefinition =>
              if func.annotations.exists(_.textMatches(AaltoInspectionBundle.message("inspection.effect.free.method.annotation.name"))) then
                ...
                if siblings.last.textMatches("}") && siblings.length > 1 then siblings = siblings.dropRight(1)
                ...
                if siblings.last != target
                  ...
                  && !parent.isInstanceOf[ScArgumentExprList]  then
